...
from lib.utils.common_coco import Human, BodyPart, CocoPart, CocoColors, CocoPairsRender

from lib.datasets import datasets_itop_rtpose
from lib.datasets import datasets_coco
...
joint_to_limb_heatmap_relationship = datasets_itop_rtpose.kp_connections(datasets_itop_rtpose.get_keypoints())
paf_xy_coords_per_limb = np.arange(2*len(joint_to_limb_heatmap_relationship)).reshape(-1, 2)
NUM_LIMBS = len(joint_to_limb_heatmap_relationship)
...
# TODO: c++ code seems hard-coded with coco parts and limbs indices
