"""
    This code evaluate the average Percentage of Correct Keypoints (PCK) over all the joints and over the whole test
    set. Specifically for the multiple-human scenarios, the PCK is calculated between the bested match prediction to
    each ground based on IOU of bounding boxes.
...
def eval_human_dataset_2d(humans_pred_set, humans_gt_set, num_joints=15, dist_th=10.0, iou_th=0.5, human_gt_set_visibility=None):
    ...
    Overall metric: average Percentage of Correct Keypoints (PCK) over all the joints and over the whole test set
...
def eval_human_dataset_2d_PCKh(humans_pred_set, humans_gt_set, head_id, neck_id, num_joints=15, h_th=0.5, iou_th=0.5, human_gt_set_visibility=None):
    """
    Evaluation of the full dataset by matching predicted humans to ground-truth humans.
    This evaluation considers multi-to-multi matching, although itop dataset only includes single person.
    For each ground-truth human, find its max overlapping predicted human vis bbox IOUs.
    ...
    for i in range(len(humans_gt_set)):
        ...
        joint_dists = match_humans_2d(humans_pred, humans_gt, iou_th)
        hsz_vec = compute_head_size(humans_gt, head_id, neck_id)
...
def match_humans_3d(humans_pred_2d, humans_gt_2d, humans_pred_3d, humans_gt_3d, iou_th=0.5):
    ...
    for i, human_gt in enumerate(humans_gt_3d):
        ...
        # invisible gt does not count distance (if to consider all GT, do not update 2D gt pos to (-1, -1))
...
if __name__ is "__main__":
    ...
    for key, value in res_data.items():
        res_data[key] = [res_data[key][ind] for ind in range(start_index, len(res_data[key]))]

    if not ablation:
        #####################################################################################
        print('\nevaluating in 2D...')
        dist_th_2d = 0.02*np.sqrt(h_org**2 + w_org**2)
        joint_avg_dist, joint_KCP = eval_human_dataset_2d(res_data['human_pred_set_2d'],
                                                          res_data['human_gt_set_2d'],
                                                          num_joints=15,
                                                          dist_th=dist_th_2d,
                                                          iou_th=0.5)
        ...
        #####################################################################################
        print('\nevaluating in 3D...')
        joint_avg_dist, joint_KCP = eval_human_dataset_3d(res_data['human_pred_set_2d'],
                                                          res_data['human_gt_set_2d'],
                                                          res_data['human_pred_set_3d'],
                                                          res_data['human_gt_set_3d'],
                                                          num_joints=15,
                                                          dist_th=0.1,
                                                          iou_th=0.5)
        joint_names = util_functions.get_keypoints()
        ...
        joint_avg_dist, joint_KCP = eval_human_dataset_2d(res_data['human_pred_set_2d_aligned'],
                                                          res_data['human_gt_set_2d'],
                                                          num_joints=15,
                                                          dist_th=dist_th_2d,
                                                          iou_th=0.5)
        ...
        joint_avg_dist, joint_KCP = eval_human_dataset_3d(res_data['human_pred_set_2d_aligned'],
                                                          res_data['human_gt_set_2d'],
                                                          res_data['human_pred_set_3d_aligned'],
                                                          res_data['human_gt_set_3d'],
                                                          num_joints=15,
                                                          dist_th=0.1,
                                                          iou_th=0.5)
        ...
        if not multiperson:
            ...
            joint_avg_dist, joint_KCP = eval_human_dataset_3d(res_data['human_gt_set_2d'],
                                                              res_data['human_gt_set_2d'],
                                                              res_data['human_pred_set_3d_perfect_2d'],
                                                              res_data['human_gt_set_3d'],
                                                              num_joints=15,
                                                              dist_th=0.1,
                                                              iou_th=0.5)
        ...
        joint_avg_dist, joint_KCP = eval_human_dataset_3d(res_data['human_pred_set_2d'],
                                                          res_data['human_gt_set_2d'],
                                                          res_data['human_pred_set_3d_read_raw_depth'],
                                                          res_data['human_gt_set_3d'],
                                                          num_joints=15,
                                                          dist_th=0.1,
                                                          iou_th=0.5)
        ...
        if not multiperson:
            ...
            joint_avg_dist, joint_KCP = eval_human_dataset_3d(res_data['human_gt_set_2d'],
                                                              res_data['human_gt_set_2d'],
                                                              res_data['human_pred_set_3d_perfect_2d_read_raw_depth'],
                                                              res_data['human_gt_set_3d'],
                                                              num_joints=15,
                                                              dist_th=0.1,
                                                              iou_th=0.5)
