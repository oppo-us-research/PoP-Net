...
DATA_DIR = '/media/yuliang/DATA/Datasets/Kinect_Depth_Human3D_small'
...
def cli():
    ...
    parser.add_argument('--output-dir', type=str,
                        default='mp_result/yolo_bbox_simple')
...
if __name__ == "__main__":
    ...
    for batch_i, (img, indices) in enumerate(val_loader):
        ...
        for b in range(args.batch_size):
            ...
            if len(bboxes[b]) == 0:
                index = indices[b]
                image_ids_out.append(int(index))
                bndbox_out.append(np.array([-1, -1, -1, -1, 0]))
            else:
                for j in range(len(bboxes[b])):
                    index = indices[b]
                    single_bbox = bboxes[b][j]
                    bndbox_out.append(single_bbox)
                    image_ids_out.append(int(index))
    ...
    np.save(args.output_dir + '/generated_bndbox_real.npy', bndbox_out)
    np.save(args.output_dir + '/generated_indices_real.npy', image_ids_out)
