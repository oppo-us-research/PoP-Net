...
class rtpose_light3d(nn.Module):
    def __init__(self, num_parts=18, num_limbs=19, num_stages=2, input_dim=3):
        ...
        # Stage 1
        # paf branch
        blocks['block1_1'] = [{'conv5_1_CPM_L': [128, 256, 3, 1, 1]},
                              {'conv5_2_CPM_L': [256, 256, 3, 1, 1]},
                              {'conv5_3_CPM_L': [256, 256, 3, 1, 1]},
                              {'conv5_4_CPM_L': [256, 128, 1, 1, 0]},
                              {'conv5_5_CPM_L': [128, 2 * num_limbs, 1, 1, 0]}]
        # heatmap branch
        blocks['block1_2'] = [{'conv5_1_CPM_S': [128, 128, 3, 1, 1]},
                              {'conv5_2_CPM_S': [128, 128, 3, 1, 1]},
                              {'conv5_3_CPM_S': [128, 128, 3, 1, 1]},
                              {'conv5_4_CPM_S': [128, 128, 3, 1, 1]},
                              {'conv5_5_CPM_S': [128, num_parts + 1, 3, 1, 1]}]

        # 3D branch (this design is only for depth data for now)
        blocks['block1_3'] = [{'conv5_1_CPM_D': [128, 128, 3, 1, 1]},
                              {'conv5_2_CPM_D': [128, 64, 3, 1, 1]},
                              {'conv5_3_CPM_D': [64, 64, 3, 1, 1]},
                              {'conv5_4_CPM_D': [64, 64, 3, 1, 1]},
                              {'conv5_5_CPM_D': [64,  num_limbs + 1, 3, 1, 1]}]

        # Stages 2
        for i in range(2, num_stages + 1):
            # paf branch
            blocks['block%d_1' % i] = [
                {'Mconv1_stage%d_L' % i: [128 + num_limbs * 2 + num_parts + 1 + num_limbs + 1, 256, 3, 1, 1]},
                {'Mconv2_stage%d_L' % i: [256, 256, 3, 1, 1]},
                {'Mconv3_stage%d_L' % i: [256, 256, 3, 1, 1]},
                {'Mconv4_stage%d_L' % i: [256, 128, 1, 1, 0]},
                {'Mconv5_stage%d_L' % i: [128, 2 * num_limbs, 1, 1, 0]}
            ]

            # heatmap branch
            blocks['block%d_2' % i] = [
                {'Mconv1_stage%d_S' % i: [128 + num_limbs * 2 + num_parts + 1 + num_limbs + 1, 128, 3, 1, 1]},
                {'Mconv2_stage%d_S' % i: [128, 128, 3, 1, 1]},
                {'Mconv3_stage%d_S' % i: [128, 128, 3, 1, 1]},
                {'Mconv4_stage%d_S' % i: [128, 128, 3, 1, 1]},
                {'Mconv5_stage%d_S' % i: [128, num_parts + 1, 3, 1, 1]}
            ]

            # 3D branch (this design is only for depth data)
            blocks['block%d_3' % i] = [
                {'Mconv1_stage%d_D' % i: [128 + num_limbs * 2 + num_parts + 1 + num_limbs + 1, 128, 3, 1, 1]},
                {'Mconv2_stage%d_D' % i: [128, 64, 3, 1, 1]},
                {'Mconv3_stage%d_D' % i: [64, 64, 3, 1, 1]},
                {'Mconv4_stage%d_D' % i: [64, 64, 3, 1, 1]},
                {'Mconv5_stage%d_D' % i: [64,  num_limbs + 1, 3, 1, 1]}
            ]
    ...
    def forward(self, x):
        ...
        out1_1 = (out1_1.sigmoid() - 0.5) * 4
        out1_2 = out1_2.sigmoid()
        out1_3 = (out1_3.sigmoid() - 0.5) * 4
        ...
        out2_1 = (out2_1.sigmoid() - 0.5) * 4
        out2_2 = out2_2.sigmoid()
        out2_3 = (out2_3.sigmoid() - 0.5) * 4

        saved_for_loss.append(out2_1)
        saved_for_loss.append(out2_2)
        saved_for_loss.append(out2_3)

        return (out2_1, out2_2, out2_3), saved_for_loss

    def _initialize_weights_norm(self):
        # TODO: this is hard-coded for 2 stages
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
...
if __name__ == "__main__":
    ...
    num_limbs = 14
    num_stages = 2
    model = rtpose_light3d(num_parts, num_limbs, num_stages, input_dim=1)
    print(model)
