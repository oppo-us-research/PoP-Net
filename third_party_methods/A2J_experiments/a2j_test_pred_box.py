...
testingImageDir = 'C:/Users/USS00019/Dataset' \
                  '/Kinect_Depth_Human3D_small/multiperson_test_v2/depth_maps'
...
with open('C:/Users/USS00019/Dataset' \
                  '/Kinect_Depth_Human3D_small/multiperson_test_v2/labels/labels_test.json') as f:
...
for key, value in ANNOTATIONS_TEST.items():
    ...
    for frame in value:
        ...
        count += 1
    keypoints2d_test_set.append(keypoints2d_test)
    keypointsUVD_test_set.append(keypointsUVD_test)
    keypoints2d_test = []
    keypointsUVD_test = []

# make sure that bndbox and image ids are generated and loaded correctly
bndbox_test = np.load('C:/Users/USS00019/Dataset' \
                 '/Kinect_Depth_Human3D_small/generated_bndbox_real.npy')
...
test_iamges_indices = np.load('C:/Users/USS00019/Dataset/Kinect_Depth_Human3D_small/generated_indices_real.npy')
image_names = list(ANNOTATIONS_TEST.keys())
for i in test_iamges_indices:
...
def compute_bbox_from_humans(humans):
    ...
    for human in humans:
        valid_joints = human
        valid_joints = np.asarray(valid_joints)
...
def main():
    ...
    count = 0
    for i in tqdm(range(len(Test2d))):
        # TODO: it it WRONG to only use subset of ground-truth for evaluation
        if i == 0:
            index = image_names.index(test_image_ids[i])
            ...
            single_img *= 255
            temp1.append(keypoints2d_test_set[index])
            temp2.append(keypointsUVD_test_set[index])
        if i != 0 and test_image_ids[i] != test_image_ids[i - 1]:
            single_img = np.load(os.path.join(testingImageDir, test_image_ids[i])).astype(np.float)
            index = image_names.index(test_image_ids[i])
            depth_max = 6
            single_img[single_img >= depth_max] = depth_max
            single_img = single_img.astype(np.float)
            single_img/= depth_max
            single_img *= 255
            temp1.append(keypoints2d_test_set[index])
            temp2.append(keypointsUVD_test_set[index])
    ...
    eval_data = {'human_pred_set_2d': pred2d_set,
                 ...
                 'human_gt_set_2d': temp1,
                 'human_gt_set_3d': temp2}
