...
def prepare_dataset_from_files(depth_data_files, depth_out_path, seg_out_path, depth_data_files_train):
    ...
    image_label_dict_train_cl = {}
    image_label_dict_train_cr = {}
    image_label_dict_train_fl = {}
    image_label_dict_train_fr = {}
    image_label_dict_train_free = {}
    image_label_dict_test_cl = {}
    image_label_dict_test_cr = {}
    image_label_dict_test_fl = {}
    image_label_dict_test_fr = {}
    image_label_dict_test_free = {}
    img_id = 0
    ...
    for depth_file in depth_data_files:
        ...
        for i in range(depth_maps.shape[0]):
            ...
            if is_train:
                ...
                if depth_file.find('_cl') != -1:
                    image_label_dict_train_cl[save_name] = [json_single_person]
                if depth_file.find('_cr') != -1:
                    image_label_dict_train_cr[save_name] = [json_single_person]
                if depth_file.find('_fl') != -1:
                    image_label_dict_train_fl[save_name] = [json_single_person]
                if depth_file.find('_fr') != -1:
                    image_label_dict_train_fr[save_name] = [json_single_person]
                if depth_file.find('_free') != -1:
                    image_label_dict_train_free[save_name] = [json_single_person]
            else:
                image_label_dict_test[save_name] = [json_single_person]

                if depth_file.find('_cl') != -1:
                    image_label_dict_test_cl[save_name] = [json_single_person]
                if depth_file.find('_cr') != -1:
                    image_label_dict_test_cr[save_name] = [json_single_person]
                if depth_file.find('_fl') != -1:
                    image_label_dict_test_fl[save_name] = [json_single_person]
                if depth_file.find('_fr') != -1:
                    image_label_dict_test_fr[save_name] = [json_single_person]
                if depth_file.find('_free') != -1:
                    image_label_dict_test_free[save_name] = [json_single_person]
    ...
    return image_label_dict, image_label_dict_train, image_label_dict_test, self_poses_mean, self_poses_std, \
           image_label_dict_train_cl, image_label_dict_train_cr, image_label_dict_train_fl, image_label_dict_train_fr, image_label_dict_train_free, \
           image_label_dict_test_cl, image_label_dict_test_cr, image_label_dict_test_fl, image_label_dict_test_fr, image_label_dict_test_free
...
def main(opt):
    ...
    out_label_file_train_cl = ops.join(label_out_path, 'labels_train_cl.json')
    out_label_file_train_cr = ops.join(label_out_path, 'labels_train_cr.json')
    out_label_file_train_fl = ops.join(label_out_path, 'labels_train_fl.json')
    out_label_file_train_fr = ops.join(label_out_path, 'labels_train_fr.json')
    out_label_file_train_free = ops.join(label_out_path, 'labels_train_free.json')

    out_label_file_test_cl = ops.join(label_out_path, 'labels_test_cl.json')
    out_label_file_test_cr = ops.join(label_out_path, 'labels_test_cr.json')
    out_label_file_test_fl = ops.join(label_out_path, 'labels_test_fl.json')
    out_label_file_test_fr = ops.join(label_out_path, 'labels_test_fr.json')
    out_label_file_test_free = ops.join(label_out_path, 'labels_test_free.json')

    image_label_dict_json, image_label_dict_train_json, image_label_dict_test_json, \
        self_pose_3d_mean, self_pose_3d_std, \
        image_label_dict_train_cl, image_label_dict_train_cr, \
        image_label_dict_train_fl, image_label_dict_train_fr, image_label_dict_train_free, \
        image_label_dict_test_cl, image_label_dict_test_cr, \
        image_label_dict_test_fl, image_label_dict_test_fr, image_label_dict_test_free\
        = prepare_dataset_from_files(depth_data_files,
                                     depth_out_path,
                                     seg_out_path,
                                     depth_data_files_train)
    ...
    # subset at different locations: train
    with open(out_label_file_train_cl, 'w') as json_file:
        json.dump(image_label_dict_train_cl, json_file, indent=4)

    with open(out_label_file_train_cr, 'w') as json_file:
        json.dump(image_label_dict_train_cr, json_file, indent=4)

    with open(out_label_file_train_fl, 'w') as json_file:
        json.dump(image_label_dict_train_fl, json_file, indent=4)

    with open(out_label_file_train_fr, 'w') as json_file:
        json.dump(image_label_dict_train_fr, json_file, indent=4)

    with open(out_label_file_train_free, 'w') as json_file:
        json.dump(image_label_dict_train_free, json_file, indent=4)

    # subset at different locations: test
    with open(out_label_file_test_cl, 'w') as json_file:
        json.dump(image_label_dict_test_cl, json_file, indent=4)

    with open(out_label_file_test_cr, 'w') as json_file:
        json.dump(image_label_dict_test_cr, json_file, indent=4)

    with open(out_label_file_test_fl, 'w') as json_file:
        json.dump(image_label_dict_test_fl, json_file, indent=4)

    with open(out_label_file_test_fr, 'w') as json_file:
        json.dump(image_label_dict_test_fr, json_file, indent=4)

    with open(out_label_file_test_free, 'w') as json_file:
        json.dump(image_label_dict_test_free, json_file, indent=4)
