...
preprocess = data_augmentation_2d3d.Compose([
    ...
    data_augmentation_2d3d.RenderDepth(cx=intrinsics['cx'], cy=intrinsics['cy'], max_ratio=1.7),
    ...
    data_augmentation_2d3d.Resize(imgWidth, imgHeight)
    ])
...
ANNOTATIONS_TRAIN = sorted(glob('{}/labels_train_*.json'.format(DATA_DIR + '/labels')))
...
ANNOTATIONS_TEST = sorted(glob('{}/labels_test_*.json'.format(DATA_DIR + '/labels')))
...
def dataPreprocess(image, anns, mode, augment=True):
    ...
    """
        Apply new data augmetation
    """
    image, anns = preprocess((image, anns))
...
val_image_datasets = my_dataloader(testingImageDir, mode='TEST', augment=True)
val_dataloaders = torch.utils.data.DataLoader(val_image_datasets, batch_size=batch_size, shuffle=False,
                                              num_workers=8, pin_memory=True)
...
def train():
    ...
    for epoch in range(nepoch):
        ...
        train_loss = train_loss_sum / TrainImgFrames
        Cls_loss = Cls_loss_sum / TrainImgFrames
        Reg_loss = Reg_loss_sum / TrainImgFrames
        print('mean train_loss_add of 1 sample: %f, #train_indexes = %d' % (train_loss, TrainImgFrames))
        print('mean Cls_loss_add of 1 sample: %f, #train_indexes = %d' % (Cls_loss, TrainImgFrames))
        print('mean Reg_loss_add of 1 sample: %f, #train_indexes = %d' % (Reg_loss, TrainImgFrames))
        ...
        if (epoch % 1 == 0):
            ...
            # validation loop
            for i, (img, label, bndbox) in enumerate(val_dataloaders):
                torch.cuda.synchronize()
                ...
                val_loss_sum = val_loss_sum + (loss.item()) * len(img)
                Cls_loss_sum = Cls_loss_sum + (Cls_loss.item()) * len(img)
                Reg_loss_sum = Reg_loss_sum + (Reg_loss.item()) * len(img)

                # printing loss info
                if i % 10 == 0:
                    print('epoch: ', epoch, ' step: ', i, '/', len(val_dataloaders), 'Cls_loss ', Cls_loss.item(), 'Reg_loss ', Reg_loss.item(),
                          ' total loss ', loss.item())

            val_loss = val_loss_sum / TestImgFrames
            Cls_loss = Cls_loss_sum / TestImgFrames
            Reg_loss = Reg_loss_sum / TestImgFrames
            print('mean val_loss of 1 sample: %f, #val_indexes = %d' % (val_loss, TestImgFrames))
            print('mean Cls_loss of 1 sample: %f, #val_indexes = %d' % (Cls_loss, TestImgFrames))
            print('mean Reg_loss of 1 sample: %f, #val_indexes = %d' % (Reg_loss, TestImgFrames))
        ...
        logging.info('Epoch#%d: total loss=%.4f, Cls_loss=%.4f, Reg_loss=%.4f, lr = %.6f'
                     % (epoch, train_loss_sum, Cls_loss_sum, Reg_loss_sum, scheduler.get_lr()[0]))
    writer.close()
