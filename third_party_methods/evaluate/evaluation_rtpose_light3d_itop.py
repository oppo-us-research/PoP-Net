...
from lib.utils.common import draw_humans_from_paf, draw_humans, paf_to_human_list
...
def cli():
    ...
    parser.add_argument('--weight', type=str,
                        default='./trained_model/rtpose_light3d_itop_init_depth/best_pose.pth')
    parser.add_argument('--output-dir', type=str,
                        default='./predictions/rtpose_light3d_itop_init_depth/')
...
if __name__ == "__main__":
    ...
    for batch_i, (img, _, _, _, _, _, _, _, indices) in enumerate(val_loader):
        for id in indices.numpy():
            human_gt = val_data.anno_dic[val_data.ids[id]]
            human_gt_2d = [human['2d_joints'] for human in human_gt]
            human_gt_set_2d.append(human_gt_2d)
            human_gt_3d = [human['3d_joints'] for human in human_gt]
        ...
        for b in range(args.batch_size):
            ...
            # read depth prediction from posedepth map network output (NOT from upsampled map)
            humans_depth = []
            for i, human in enumerate(humans_2d):
                ...
                for j, joint in enumerate(human):
                    if visibility[i][j] > 0.5:
                        ...
                        # TODO: why using 'round' leads to worse result than 'int'
                        # x2d = round(joint[0] / cfg.MODEL.DOWNSAMPLE)
                        # y2d = round(joint[1] / cfg.MODEL.DOWNSAMPLE)
                        # x2d = min(max(x2d, 0), int(args.input_size / cfg.MODEL.DOWNSAMPLE) - 1)
                        # y2d = min(max(y2d, 0), int(args.input_size / cfg.MODEL.DOWNSAMPLE) - 1)
                        # human_depth[j] = posedepth[b][y2d, x2d, joint2chn[j]]
            ...
            for i, human in enumerate(humans_2d):
                ...
                human[np.where(visibility[i]), 0] = human[np.where(visibility[i]), 0] / cfg.MODEL.IMAGE_SIZE[0] * args.w_org
                human[np.where(visibility[i]), 1] = human[np.where(visibility[i]), 1] / cfg.MODEL.IMAGE_SIZE[1] * args.h_org
                humans_2d[i] = human
