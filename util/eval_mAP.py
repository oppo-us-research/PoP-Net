"""
    This is the python implementation of the mAP evaluation originated from MPII dataset. The mAP evaluation is
    particularly designed for multiple human scenarios.

    The original matlab code refers to:
        http://human-pose.mpi-inf.mpg.de/#evaluation

    What's new from this code:
        1. python implementation of the 2D mAP evaluation following h-0.5 rule
        2. 3D mAP evaluation following 10cm rule
...
if __name__ is "__main__":
    ...
    AP_2D = eval_ap_mpii_v2(res_data['human_pred_set_2d_aligned'], res_data['human_pred_set_part_conf'],
                            res_data['human_gt_set_2d'], gt_visibility_set=[],
                            head_id=0, neck_id=1, joint_names=joint_names, thresh=0.5)

    AP_3D = eval_ap_3D(res_data['human_pred_set_3d_aligned'], res_data['human_pred_set_part_conf'],
                       res_data['human_gt_set_3d'], gt_visibility_set=[], joint_names=joint_names, thresh=0.1)
