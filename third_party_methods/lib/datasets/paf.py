...
def putVecMaps(centerA, centerB, accumulate_vec_map, count, grid_y, grid_x, stride):
    ...
    # modified to include ymin=ymax or xmin=xmax
    min_x = max(int(round(min(centerA[0], centerB[0]) - thre)), 0)
    max_x = min(int(round(max(centerA[0], centerB[0]) + thre)), grid_x-1)
    min_y = max(int(round(min(centerA[1], centerB[1]) - thre)), 0)
    max_y = min(int(round(max(centerA[1], centerB[1]) + thre)), grid_y-1)
