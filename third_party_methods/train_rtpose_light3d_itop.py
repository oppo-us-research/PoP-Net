...
from lib.network.losses import rtpose_light3d_loss, rtpose_light3d_loss_fgweight
from lib.datasets import data_augmentation_2d3d, datasets_itop_rtpose
...
DATA_DIR = '/raid/yuliangguo/Datasets/ITOP'
# DATA_DIR = '/media/yuliang/DATA/Datasets/ITOP'
if not os.path.exists('./trained_model'):
    os.mkdir('./trained_model')
save_folder = './trained_model/rtpose_light3d_itop'
...
def train_factory(args, preprocess):
    train_datas = [datasets_itop_rtpose.ItopKeypoints(
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (img, heatmap_target, paf_target, posedepth_target, fg_masks, _, _, _, _) in enumerate(train_loader):
...
def validate(val_loader, model, epoch):
    ...
    for i, (img, heatmap_target, paf_target, posedepth_target, fg_masks, _, _, _, _) in enumerate(val_loader):
        # measure data loading time
        data_time.update(time.time() - end)
        img = img.cuda()
        heatmap_target = heatmap_target.cuda()
...
if __name__ == "__main__":
    ...
    preprocess = data_augmentation_2d3d.Compose([
        ...
        data_augmentation_2d3d.Rotate(),
        data_augmentation_2d3d.RenderDepth(),
        data_augmentation_2d3d.Hflip(swap_indices=datasets_itop_rtpose.get_swap_part_indices()),
